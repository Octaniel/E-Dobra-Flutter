...
class SegurancaProvider {
  ...
  Future<List<UsuarioModel>> getAll() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        var listUsuarioModel = jsonResponse.map<UsuarioModel>((map) {
  ...
  Future<UsuarioModel> getId(int id) async {
  ...
  Future<bool> login(String senha, String email) async {
    ...
    if (response.statusCode == 200) {
      ...
      storage.setString(
          "pessoaId", decode["cliente"]["pessoa"]['id'].toString());
  ...
  Future<bool> add(UsuarioModel obj) async {
...
//    await storage.clear();
  ...
  Future<bool> edit(UsuarioModel obj) async {
    try {
